...
export default function IVRPacketPanel({ patientId, locationId, visitId }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Try common table names and shapes (won‚Äôt break if missing)
  ...
  const summaryStats = useMemo(() => {
    ...
    return wounds.map((w) => {
      ...
      return {
        ...
        type: w.wound_type,
        stage: w.stage,
        lwd: [w.length_cm, w.width_cm, w.depth_cm].map((x) => (x == null ? "-" : x)).join(" √ó "),
        area,
        exudate: w.exudate,
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {summaryStats.slice(0, 6).map((w, idx) => (
                  <div key={`${w.label}-${idx}`} className="muted" style={{ fontSize: 12 }}>
                    <strong>{w.label}</strong> ï {w.site || "-"} ï L√óW√óD: {w.lwd}
                    {w.area != null ? ` ï Area: ${w.area} cm¬≤` : ""}
                  </div>
                ))}
              </div>
            ) : (
          ...
          <PacketCard title="Packet Draft (editable)">
            <textarea
              ...
              placeholder="Click ‚ÄúBuild Packet Draft‚Äù to generate‚Ä¶"
              style={{ width: "100%", minHeight: 520, whiteSpace: "pre-wrap" }}
            />
            <div className="space" />
            <div className="muted" style={{ fontSize: 12 }}>
              Next step (we‚Äôll add): ‚ÄúExport PDF‚Äù and ‚ÄúAttach to IVR submission‚Äù buttons.
            </div>
          </PacketCard>
        </div>
      )}
